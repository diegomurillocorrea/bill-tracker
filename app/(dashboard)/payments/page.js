...
export default async function PaymentsPage() {
  ...
  const [paymentsResult, paymentMethodsResult] = await Promise.all([
    supabase
      .from("payments")
      .select(
        "id, receipt_id, payment_method_id, total_amount, created_at, receipts(id, account_receipt_number, clients(name, last_name, phone_number), services(name)), payment_methods(name)"
      )
